...
# Testbenches and other provided files may live directly in ../solution/, in
# ../solution/tb/ (the published testbench that `make test` runs against the
# student DUT), or in ../solution/ref/ (decrypted reference sources). Search
# all three so a starter can pull the file regardless of where the exercise
# happens to keep it.
$(FETCH_FROM_SOLUTION):
	@src=""; \
	for cand in "../solution/$@" "../solution/tb/$@" "../solution/ref/$@"; do \
		if [ -f "$$cand" ]; then src="$$cand"; break; fi; \
	done; \
	if [ -z "$$src" ]; then \
		echo "ERROR: $@ not found in ../solution/, ../solution/tb/, or ../solution/ref/" >&2; \
		exit 1; \
	fi; \
