...
Run from anywhere in the repo. It stashes your local edits (including new files), pulls the latest course updates, then re-applies your edits on top. If anything conflicts, your work stays safely in a stash and the script prints exactly how to recover it — nothing is ever lost. The rest of this section explains what it does by hand, in case you want to do it step by step.
...
When you've finished a lab day, bundle everything to hand in with one command (pass the **day number**):

```bash
scripts/lab_submit.sh 2        # bundles lab day 2
```

It produces `day02_submission.zip` in the repo root containing:

- **`reflection.txt`** — your short write-up. The first time you run the command it creates a template in the day folder and asks you to fill it in (a couple of sentences) and re-run. A blank or one-line reflection is rejected.
- **`sources/…`** — every `.v` / `.sv` file you modified or added in that day's folder (detected via git, so untouched starter files and auto-fetched helpers are skipped).
- **`waveforms/…`** — an SVG waveform image rendered from each `*.vcd` your simulations produced. **Run `make exN_sim` first** so a `.vcd` exists; otherwise the bundle is built without a waveform and warns you.

Open the `.svg` waveforms in any web browser to double-check them before submitting.
...
## 6. TL;DR — Every Session

```bash
cd ~/path/to/hdl-for-dsd
scripts/lab_sync.sh          # pull updates, keep your work (see Section 3)
