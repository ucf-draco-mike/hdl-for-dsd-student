...
module tb_counter;
    ...
    initial begin
        ...
        if (count !== 4'h4) begin
            fail_count = fail_count + 1;
            $display("FAIL Hold: count changed to %h (expected 4)", count);
        end else
            $display("PASS Hold: count stayed at 4");

        repeat (4) @(posedge clk); #1;
        test_count = test_count + 1;
        if (count !== 4'h4) begin
            fail_count = fail_count + 1;
            $display("FAIL Hold 5cyc: count=%h (expected 4)", count);
        end else
            $display("PASS Hold 5cyc: count=4 after 5 disabled cycles");

        // Test 4: Rollover F->0
        reset = 1;
        // Drive stimulus one tick *after* the edge so the DUT samples the
        // intended reset/enable values instead of racing this assignment.
        @(posedge clk); #1; reset = 0; enable = 1;
